"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const HTTP_RESPONSE = {
    SUCCESS_CODE: 200,
    CREATED_CODE: 201,
    ACCEPTED_CODE: 202,
    ERROR_FROM_USER_CODE: 400,
    UNAUTHORIZED_CODE: 401,
    ERROR_PAYMENT_CODE: 402,
    FORBIDDEN_CODE: 403,
    PAGE_NOT_FOUND_CODE: 404,
    METHOD_NOT_ALLOWED_CODE: 405,
    INTERNAL_ERROR_CODE: 500,
    NOT_IMPLEMENTED_CODE: 501,
    MESSAGE: {
        SUCCESS_CODE: 'The request is done successfully!',
        ERROR_FROM_USER_CODE: 'Error from user, please check your api parameters!',
        ERROR_FROM_USER_CODE_1: 'We were not able to parse your path request,\n please check our API again to learn our pattern!',
        UNAUTHORIZED_CODE: 'The user is not logged in, please login first!',
        ERROR_PAYMENT_CODE: 'There is no Access-Key or Not Valid in Header!',
        FORBIDDEN_CODE: 'The key already exist! You have to update, or delete the key then create again!',
        PAGE_NOT_FOUND_CODE: 'The path does not exists!',
        INTERNAL_ERROR_CODE: 'Sorry, there is an error from database server due to your request, contact the supplier!',
        NOT_IMPLEMENTED_CODE: 'Sorry, the request with your parameters are not yet implemented, contact the supplier!',
    },
};
exports.default = HTTP_RESPONSE;
